...
## 示例四：缺槽/歧义时的澄清（不进入 Planner，多轮补全后再规划）

**场景**：用户首句信息不足或存在歧义，意图理解模块检测到**必填槽位缺失或歧义**，返回 `needs_clarification=true` 与澄清问句，**不进入 Planner**；用户补充后再次走意图理解（或仅补槽），槽位齐全后再进入 Planner 与执行。
...
| 步骤 | 模块在做的事 | 输入 | 输出/中间产物 |
| --- | --- | --- | --- |
| **2.1 意图分类** | 判断属于「内容相关」意图，但无法区分是「选题策划」还是「内容生成」或「选题+成稿」 | 用户原文 | 意图类型：可能为 `content_generation` 或混合，**存在歧义**（做点内容 = 只出选题？只出正文？还是选题+正文？） |
| **2.2 OOS 检测** | 判断在预定义意图范围内 | 用户原文 | 在范围内，非 OOS |
| **2.3 槽位抽取** | 从「做点内容」中无法抽取行业、账号阶段、目标、内容形式、参考等 | 用户原文 + Slot Schema | `slots` 多为空或仅少量推断：如 `content_form=？（未明确图文/视频）` |
| **2.4 必填槽位检查** | 按当前意图（或歧义下的多可能意图）检查必填槽：行业/账号阶段/目标/内容形式等 | 意图类型 + slots + 必填配置 | **必填槽缺失**：行业缺、目标缺、内容形式歧义；**意图歧义**：「做点内容」未区分选题 vs 成稿。 |
| **2.5 生成澄清问句** | 根据缺失项与歧义项生成 1～N 个澄清问句，供前端展示给用户 | 缺失槽位列表 + 歧义说明 | `needs_clarification=true`；`clarification_question` 或 `clarification_questions[]`；可选 `missing_slots`、`ambiguous_intent` |
| **2.6 输出结构化结果（不进入 Planner）** | 汇总为「待澄清」的 JSON；**不调用 Planner**，等待用户回复 | 以上全部 | 见下方「第一轮意图输出」 |

**第一轮意图模块输出（示例）：**

```json
{
  "demand_summary": "用户希望「做点内容」，但当前信息不足，无法确定具体是选题策划、内容生成或完整链路，且缺少行业、目标等必填信息。",
  "intent_type": "ambiguous",
  "intent_breakdown_candidates": ["topic_planning", "content_generation"],
  "slots": {
    "industry": null,
    "goal": null,
    "content_form": null,
  ...
  "clarification_question": "您是想先做「选题」（只出选题方向），还是直接「写一篇笔记」？另外请简单说下您的行业或账号方向（例如美妆、美食、装修），方便我按您的定位来出内容。",
  "missing_slots": ["industry", "goal", "content_scope"],
  "ambiguous_intent": "做点内容 可能指：仅选题 / 仅成稿 / 选题+成稿"
}
```

（若产品支持多问句，可用 `clarification_questions` 数组分别问「选题还是成稿」「行业/账号方向」等。）

### 3. 系统行为：展示澄清问句，等待用户回复

- 前端根据 `needs_clarification=true` **不进入 Planner**，仅展示 `clarification_question`（或逐条展示 `clarification_questions`）。
- 用户回复示例：「先出选题吧，我们是做母婴的，想涨粉。」

### 4. 第二轮：用户补充后的意图理解

- **输入**：用户首句「帮我做点内容。」+ 用户补充「先出选题吧，我们是做母婴的，想涨粉。」（或拼接为多轮对话上下文）

| 步骤 | 模块在做的事 | 输入 | 输出/中间产物 |
| --- | --- | --- | --- |
| **2.1 意图分类** | 结合首句与补充句，确定为「选题策划」；目标为涨粉 | 多轮上下文 | 意图类型：`topic_planning`；目标：涨粉 |
| **2.2 槽位抽取** | 从补充句抽取：行业=母婴，目标=涨粉，内容范围=仅选题 | 多轮上下文 + 首轮 partial slots | `slots`: `industry=母婴`, `goal=涨粉`, `content_scope=仅选题` |
| **2.3 必填槽位检查** | 再次检查当前意图的必填槽 | 意图 + slots | 必填已齐全（行业、目标、范围已明确） |
| **2.4 输出** | 输出 `needs_clarification=false`，进入下游 | 以上 | 见下方「第二轮意图输出」 |

**第二轮意图模块输出（示例）：**

```json
{
  "demand_summary": "母婴行业账号，目标涨粉，希望先做选题策划（仅出选题方向）。",
  "intent_type": "topic_planning",
...
### 5. Planner 与下游（仅在第二轮槽位齐全后执行）

- **Planner**：接收第二轮意图输出 + 账号上下文，生成计划，例如仅一步「选题策划」：基于行业=母婴、目标=涨粉，做热门话题捕捉与选题价值评估，输出若干高优先级选题及执行建议。
- **执行**：按计划调用选题策划 Agent，将选题列表返回用户；本例不包含内容生成与发布。

**小结（示例四）**：缺槽或歧义时，**询问用户澄清需求**发生在**意图理解环节**；系统不进入 Planner，仅做多轮澄清与槽位补全，待 `needs_clarification=false` 后再进入 Planner 与执行。

---

## 示例五：需求合理且明确，但因法规/技术/成本等原因不能执行

**场景**：用户需求**合理且明确**（意图与槽位均清晰、无缺槽无歧义），但系统因**法规、技术能力、成本**等原因**无法执行**。此时不做「澄清」，而是做**可执行性检查**，向用户说明不能执行的原因，并可选提供替代方案或转人工。

### 1. 用户输入

- **原始输入**：「帮我把上一篇笔记投流，预算 5000，定向 25～35 岁女性。」

### 2. 意图识别做的所有事情

| 步骤 | 模块在做的事 | 输入 | 输出/中间产物 |
| --- | --- | --- | --- |
| **2.1 意图分类** | 判断属于「投流」意图；与投流 Agent 对齐 | 用户原文 | 意图类型：`ads_planning`（投流）；单意图 |
| **2.2 OOS 检测** | 判断在预定义意图范围内 | 用户原文 | 在范围内，非 OOS |
| **2.3 槽位抽取** | 抽取：参考=上一篇笔记、预算=5000、定向=25～35岁女性 | 用户原文 + Slot Schema | `slots`: `reference=上一篇笔记`, `budget=5000`, `targeting=25～35岁女性` |
| **2.4 必填槽位检查** | 检查投流意图的必填槽 | 意图 + slots | 必填齐全；**无缺槽、无歧义** |
| **2.5 输出意图结果** | 输出结构化需求；**不在此环节判断「能否执行」** | 以上 | `demand_summary`、`intent_type`、`slots`、`needs_clarification=false` |

**意图模块输出（示例）：**

```json
{
  "demand_summary": "用户希望对上一篇笔记进行投流，预算 5000 元，定向 25～35 岁女性。",
  "intent_type": "ads_planning",
  "slots": {
    "reference": "上一篇笔记",
    "budget": 5000,
    "targeting": "25～35岁女性"
...
在意图输出之后、进入 Planner 或执行之前，增加**可执行性检查**环节（可与意图模块同管道输出，或由独立「策略/网关」节点执行）：

| 步骤 | 检查在做的事 | 输入 | 输出/中间产物 |
| --- | --- | --- | --- |
| **3.1 法规与资质** | 判断当前请求是否满足平台与合规要求（如小红书投流需报白/资质、广告法限制等） | 意图 + slots + 账号资质/配置 | 本例假设：**当前账号未开通官方投流资质** → 不可执行，原因=法规/资质 |
| **3.2 技术能力** | 判断当前系统是否具备执行能力（如是否已接入官方投流 API、是否支持该定向） | 意图 + 系统能力配置 | 本例假设：**未接入官方投流 API** 或 **该定向暂不支持** → 不可执行，原因=技术能力 |
| **3.3 成本与策略** | 判断是否在成本或策略允许范围内（如单笔预算上限、服务未开放投流等） | 意图 + slots + 成本/策略配置 | 可选：预算超限或当前套餐不支持投流 → 不可执行，原因=成本/策略 |
| **3.4 输出不可执行结果** | 汇总不能执行的原因与面向用户的说明；可选：替代方案、转人工入口 | 以上 | `can_fulfill=false`、`reason`、`message_to_user`、可选 `alternative`、`transfer_to_human` |
...
```json
{
  "can_fulfill": false,
  "reason": "regulation",
  "reason_detail": "当前账号未完成小红书官方投流资质（报白/开户），或系统暂未接入官方投流 API，无法执行投流操作。",
  "message_to_user": "您的投流需求（上一篇笔记、预算 5000、定向 25～35 岁女性）已理解。因平台合规要求，当前账号需先完成小红书官方投流资质开通，且本系统需接入官方投流能力后才能执行。您可先完成账号资质开通，或联系客服由人工协助投流。",
  "alternative": "您可先使用「选题策划」或「内容生成」产出更多笔记，待投流能力开通后再对优质笔记进行投流。",
  "transfer_to_human": true,
  "blocked_step": "投流"
}
```

（若原因仅为**成本**（如预算超限），可 `reason: "cost"`；若为**技术能力**（如 API 未接入），可 `reason: "capability"`。）

### 4. 下游实际发生的事（不进入执行）

- **不进入 Planner** 或 **进入 Planner 但仅产出「不可执行」说明**：当 `can_fulfill=false` 时，系统**不调用投流 Agent**，不执行实际投流。
- **前端/产品**：根据 `can_fulfill`、`message_to_user`、`alternative`、`transfer_to_human` 向用户展示说明，并可提供「转人工」或「替代方案」入口。
- **与「澄清」的区别**：本例**不**是缺槽或歧义，因此**不**使用 `needs_clarification`；用户需求已被完整理解，只是**能力/法规/成本**上不能执行，故用独立的 `can_fulfill` + `reason` + `message_to_user` 表达。

### 5. 可选：部分可执行时的处理

若用户请求为混合意图（如「先出选题，再投流」），且仅**部分步骤**不可执行（如选题可执行、投流不可执行），可：

- 意图与可执行性检查后，输出 `can_fulfill_partial: true`、`executable_steps: ["选题策划"]`、`blocked_steps: ["投流"]`；
- Planner 仅对可执行步骤生成计划并执行，执行结束后向用户说明「选题已完成；投流因 xxx 暂无法执行，建议 xxx」。

**小结（示例五）**：当用户需求**合理且明确**但系统因**法规、技术能力、成本**等原因**不能执行**时，在意图理解之后（或与意图同管道）做**可执行性检查**，输出 `can_fulfill=false`、原因及面向用户的说明，**不进入执行**；可选提供替代方案或转人工。这与「缺槽/歧义」的澄清环节区分开：澄清是**补全需求**，不可执行是**说明能力边界**。
...
| **需求合理且明确但不可执行** | 意图输出后做**可执行性检查**（法规/资质、技术能力、成本）：若不能执行则输出 `can_fulfill=false`、`reason`、`message_to_user`，不进入 Planner/执行；可选 `alternative`、`transfer_to_human`。见**示例五**。 | 若 Planner 或执行中才发现不可执行，可提前在可执行性检查阶段拦截并统一返回说明。 |

五个示例分别覆盖：**示例一**——流行选题 → 特色图文 → 用户可调整 → 发布小红书（工装企业，含客户配置模式1/2、长短期记忆、用户可修改任务拆解）；**示例二**——混合意图（起号+种草+节奏）；**示例三**——单意图（爆款标题生成）；**示例四**——缺槽/歧义时的澄清（首句信息不足 → 意图理解返回 needs_clarification + 澄清问句 → 不进入 Planner → 用户补充后第二轮意图补全槽位 → 再进入 Planner 与执行）；**示例五**——需求合理且明确但因法规/技术/成本不能执行（意图与槽位完整 → 可执行性检查 → can_fulfill=false + reason + message_to_user → 不进入执行，可选替代方案或转人工）。均展示了意图识别与 Planner 在上述流程中**分别做的所有事情**及中间产物形态。
